...
def onlyOneEntityQuery(entity):
    ...
    for i in res:
        # name是字符串   type是列表
        if i['b']['name'] != None:
            r = {'name': i['b']['name'], 'symbolSize': 60, 'type': ",".join(list(i['b'].labels)), 'c': 1 }
            data.append(r)
    # 获取关系
    for k in res:
...
def oneOptionAndOneEntityQuery(entity, option):
    ...
    for i in res:
        # name是字符串   type是列表
        r = {'name': i['b']['name'], 'symbolSize': 30, 'c': 1, 'type': ",".join(list(i['b'].labels)) }
        data.append(r)
    # 获取关系
    for k in res:
...
def cateDegreeInfo(province_name, year):
    ...
    return list(set([ r['n.category'] for r in res ])), list(set([ k['n.degree'] for k in res ]))
