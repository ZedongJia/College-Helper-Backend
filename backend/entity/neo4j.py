...
def universitySpecial(university):
    ...
    for major_dict in major:
        data.append(
            {"name": major_dict["m.name"], "symbolSize": 60, "c": 1, "type": "major"}
        )
        link.append(
            {"source": university, "label": "HAS", "target": major_dict["m.name"]}
        )
...
def canBack(university, major):
    ...
    if len(major_u_id) == 0:
        ...
        # 有关系
        return [
            {"name": university, "symbolSize": 60, "c": 1, "type": "university"},
            {"name": major, "symbolSize": 60, "c": 1, "type": "major"},
        ], [{"source": university, "label": "HAS", "target": major}]
...
def cateDegreeInfo(province_name, year):
    ...
    return list(set([r["n.category"] for r in res])), list(
        set([k["n.degree"] for k in res])
    )
...
def ScoreRecommend(province, myScore):
    ...
    for item_dict in res:
        item_dict["m.ruanKeScore"] = (
            "暂无数据" if item_dict["m.ruanKeScore"] == "" else item_dict["m.ruanKeScore"]
        )
        if item_dict["m.fk_university_id"] not in universitys_id_name:
            universitys_id_name[item_dict["m.fk_university_id"]] = item_dict[
                "m.fk_university_id"
            ]
    # 再次查询，获取对应大学的名称
    for i in universitys_id_name.keys():
        universitys_id_name[i] = conn.run(
            "match (n:university) where n.id = %d return n.name" % (i,)
        ).data()[0]["n.name"]
...
def aiTwoEntityQuery(entity_name, entity_type, num, label):
    ...
    if len(res) != 0:
        for major_dict in res:
            ...
            link.append(
                {"source": entity_name, "label": label, "target": major_dict["m.name"]}
            )
    NEO4j_POOL.free(conn)
    return data, link
