...
def recommendation(request):
    ...
    try:
        conn = py2neo.Graph(
            profile=NEO_DB_POOL["profile"],
            name=NEO_DB_POOL["username"],
            password=NEO_DB_POOL["password"],
        )
...
def getScoreInfo(request):
    ...
    return JsonResponse({"detail": detail, "keys": keys}, safe=False)
...
def AIChat(request):
    ...
    try:
        if (score != None or rank != None) and province is None:
            ...
            for item_dict in detail_dict:
                relationList.append(
                    "学校名称：" + item_dict["name"] + "，专业名称：" + item_dict["m.name"]
                )
                ...
                if num >= 10:
                    break
            relationList.append(
                "请你根据考生的所在地以及向往的大学以及以上可能感兴趣的专业，并参考当前国内各专业的热度情况、薪资情况等因素，为该学生提供合理的专业选择建议。如果成绩不太理想的话，你要适度安慰该考生。"
            )
        ...
        # 未输入 分数/排名/大学 信息  ==>  不查可以去的 大学/专业  ==>  xxx省有什么大学 | xxx大学有什么特色，招生政策。。。 |
        elif (province != None and len(uni_) == 0) or (
            province == None and len(uni_) != 0
        ):
            ...
            if sentence.find("人") > -1:
                ...
                data, link, relationList = ai.onlyOneEnity(
                ...
                data, link = neo4j.twoEntityQuery(
                    nameAndLabel[0]["name"], nameAndLabel[1]["name"], 1
                )
            # 关系太多，直接调用接口
            else:
