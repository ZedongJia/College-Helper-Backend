...
def queryEntity(request):
    ...
    if label == "university":
        ...
        try:
            ...
            data = cursor.data()[0]["a"]
            cursor = conn.run(
                cypher="match (a: %s)<-[r:HAS]-(b:university) where a.name=$name return r, b limit 5"
                % label,
                params={"name": name},
            )
            rel = cursor.data()
        ...
        try:
            ...
            if label == "person":
                ...
                cursor = conn.run(
                    cypher="match (a: %s)-[:BELONG_TO]->(b:university) where a.name=$name return b"
                    % label,
                    params={"name": name},
                )
                university = cursor.data()[0]["b"]
                cursor = conn.run(
                    cypher="match (a: %s)-[:BELONG_TO]->(b:university) where b.name=$name return a.name, a.identity limit 5"
                    % label,
