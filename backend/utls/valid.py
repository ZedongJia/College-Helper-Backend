...
class Validate:
    ...
    def get_account(request):
        return request.get_signed_cookie(key="account", default=-1, salt=Validate.SALT)
    ...
    def _set_cookie(self):
        self.response.set_signed_cookie(key='uuid', value=self.uuid, salt=self.SALT, max_age=self.EXISTING_DURATION)
        self.response.set_signed_cookie(key='account', value=self.account, salt=self.SALT, max_age=self.EXISTING_DURATION)

    @staticmethod
    def valid(request):
        # 取出cookie存储信息
        uuid = request.get_signed_cookie(key="uuid", default=-1, salt=Validate.SALT)
        account = request.get_signed_cookie(key="account", default=-1, salt=Validate.SALT)
